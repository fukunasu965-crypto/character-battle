...
function animateFighter(index,type){
 ...
 void w.offsetWidth;w.classList.add("anim-"+type);setTimeout(()=>w.classList.remove("anim-"+type),700)
}
...
function readManual(){return{name:$("char-name").value.trim()||"探索者",maxHp:val("char-hp"),str:val("char-str"),dex:val("char-dex"),attack:val("char-atk"),dodge:val("char-dodge"),firstAid:val("char-aid"),image:imageData.p1,attacks:[{name:"攻撃",skill:val("char-atk"),damage:$("char-dmg").value||"1d4+1",kind:"damage"}]}}
function applyForm(c){$("char-name").value=c.name;if(c.image){imageData.p1=c.image;$("char-image-preview").innerHTML=`<img src="${c.image}">`;}$("char-hp").value=c.maxHp;$("char-str").value=c.str||50;$("char-dex").value=c.dex;$("char-atk").value=c.attack;$("char-dodge").value=c.dodge;$("char-aid").value=c.firstAid;if(c.attacks?.[0])$("char-dmg").value=c.attacks[0].damage||"1d4+1"}
...
function render(){
 ...
 $("stats"+n).innerHTML=stat("STR",c.str,c.str)+stat("攻撃",c.skills.attack,effectiveAttack(c))+stat("回避",c.skills.dodge,effectiveDodge(c,observed))+stat("反撃",c.skills.attack,effectiveCounter(c));
