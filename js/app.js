  ...
  try{
   if(msg.type==="request-character"&&!hostSide){
    ...
    battle={
      ...
      round:1,ap:1,pending:null,gameOver:false,
...
function comStep(){
 ...
 setTimeout(()=>{
  ...
  /* Reaction: preserve HP aggressively. Counter only when it has a strong chance. */
  if(battle.pending){
  ...
  /* If taunted, it must attack. Prefer heavy when it can finish or has spare AP. */
  if(c.state.taunted){
  ...
  /* Disruption first: stack pressure before attacking. */
  ...
  /* Analyze is now a core setup move, especially against good dodgers. */
  if(!c.state.attackBonus && (t.dodge>=45 || c.attack<65) && Math.random()<.72)return action("analyze");

  /* Grapple to deny the opponent's next AP recovery. */
  if(ap>=1 && c.grapple>=50 && c.str>=t.str && !t.state.nextApPenalty && Math.random()<.58)
   return action("grapple");

  /* Re-apply disruption whenever the opponent is currently clean. */
  if(canIntimidate && Math.random()<.48)return action("intimidate");
  if(canTaunt && Math.random()<.42)return action("taunt");

  /* Convert setup into damage. */
  if(ap>=2 && (c.state.attackBonus || targetHp<=.5 || ap>=3) && Math.random()<.72)return action("heavy");
  if(ap>=1)return action("attack");

  /* AP0 actions ensure COM never wastes a turn. */
...
function deal(extra=0){
 ...
 if(p.type==="grapple"){
  ...
  let n=rawDamage(p)+(p.spirit||0)+(p.tauntDamage||0)+extra;
 ...
 const n0=rawDamage(p)+(p.spirit||0)+(p.tauntDamage||0)+extra;let n=n0;
 if(d.state.guard){if(p.type==="heavy"){battle.log.push({text:"💥 ガードブレイク！",cls:"special"})}else{n=Math.max(0,n-2);d.state.guard=false}}
...
function attack(type){
 ...
 const taunted=!!c.state.taunted;if(taunted)c.state.taunted=false;
 const w=selected(c),skill=clamp(atkSkill(c,type)-activePenalty(c)-(taunted?10:0)),observed=c.state.attackBonus>0,spirit=c.state.spirit;c.state.attackBonus=0;c.state.spirit=0;if(type==="normal")c.state.normalAttacks++;battle.ap-=cost;
 ...
 if(z.type==="oneCritical")battle.ap+=1;if(z.type==="hundredFumble"){battle.ap=0;c.state.rp=0;return finishAction()}if(z.type==="fumble"){c.state.rp=Math.max(0,c.state.rp-1);return finishAction()}if(z.type==="failure")return finishAction();
...
function grappleAttack(){
 ...
 const taunted=!!c.state.taunted;if(taunted)c.state.taunted=false;
 const skill=clamp(w.skill+c.state.attackBonus-activePenalty(c)-(taunted?10:0));const observed=c.state.attackBonus>0;c.state.attackBonus=0;battle.ap--;
 ...
 if(z.type==="oneCritical")battle.ap+=1;if(z.type==="hundredFumble"){battle.ap=0;c.state.rp=0;return finishAction()}if(z.type==="fumble"){c.state.rp=Math.max(0,c.state.rp-1);return finishAction()}if(z.rank<3)return finishAction();
...
function action(id){
 ...
 if(id==="taunt"){
  ...
  battle.log.push({text:`挑発 APP×5 ${r}/${skill} → ${z.text}`,cls:"special"});
  if(z.rank>=3){foe().state.taunted=true;battle.log.push({text:`🔥 ${foe().name}は挑発された！ 次の行動は攻撃系のみ / 与ダメ+2 / 命中-10。`,cls:"special"})}
  return finishAction();
 }
 if(id==="analyze"){
  const skill=clamp(specialSkill(c,"int")-activePenalty(c)),r=rollD100(),z=judgeRoll(r,skill);showDiceFx(r,skill,z,"ANALYZE / INT×5");
  battle.log.push({text:`分析 INT×5 ${r}/${skill} → ${z.text}`,cls:"special"});
  if(z.rank>=3){c.state.attackBonus=20;c.state.analyzed=true;battle.log.push({text:`👁 分析成功！ 次の攻撃+20 / 相手回避-15。`,cls:"special"})}
  return finishAction();
 }
 if(id==="intimidate"){
  if(c.mp<2)return;
  ...
  battle.log.push({text:`威圧 POW×5 ${r}/${skill} → ${z.text} / MP-2`,cls:"special"});
  if(z.rank>=3){foe().state.intimidated=true;battle.log.push({text:`⚡ ${foe().name}は萎縮した！ 次の能動判定-10。`,cls:"special"})}
  return finishAction();
 }
 if(id==="guard"&&battle.ap>=1){c.state.guard=true;battle.ap--;battle.log.push({text:`${c.name}は防御態勢。`});finishAction()}
  if(id==="heal"&&battle.ap>=2){battle.ap-=2;const healSkill=clamp(c.skills.firstAid-activePenalty(c)),r=rollD100(),z=judgeRoll(r,healSkill);showDiceFx(r,healSkill,z,"FIRST AID");let h=z.type==="oneCritical"?5:z.type==="critical"?rollDice(3)+2:z.type==="success"?rollDice(3):0;if(h)c.hp=Math.min(c.maxHp,c.hp+h);if(z.type==="hundredFumble")c.hp=Math.max(1,c.hp-2);battle.log.push({text:`応急手当 ${r}/${healSkill} → ${z.text}${h?` / HP+${h}`:z.type==="hundredFumble"?" / HP-2":""}`});if(z.type==="oneCritical")battle.log.push({text:"🌟 完璧な応急処置！ HPを5回復。",cls:"special"});else if(z.type==="critical")battle.log.push({text:"✨ 的確な応急処置！ 回復量が1D3+2に強化。",cls:"special"});else if(z.type==="hundredFumble")battle.log.push({text:"☠ 応急手当で100ファンブル！ 処置を誤りHP-2（HP1未満にはならない）。",cls:"special"});else if(z.type==="fumble")battle.log.push({text:"💀 応急手当に失敗。回復は発生しない。",cls:"special"});finishAction()}
...
function render(){
 ...
 ["attack","heavy","grapple","guard","analyze","taunt","intimidate","heal"].forEach(id=>$(id).disabled=!!battle.pending||battle.gameOver||!canActHere());
 if(!battle.pending){
 $("attack").disabled||=battle.ap<1;$("heavy").disabled||=battle.ap<2;$("grapple").disabled||=battle.ap<1||!g;$("guard").disabled||=battle.ap<1;$("heal").disabled||=battle.ap<2;
 $("intimidate").disabled||=c.mp<2;
 if(c.state.taunted){["guard","analyze","taunt","intimidate","heal"].forEach(id=>$(id).disabled=true)}
}
...
setOnlineStatus("オンライン：操作できます / BUILD 2.43");
...
function resultReturnToLobby(ev){
 ...
 try{setOnlineStatus("オンライン：操作できます / BUILD 2.43")}catch(e){}
 window.scrollTo(0,0);
